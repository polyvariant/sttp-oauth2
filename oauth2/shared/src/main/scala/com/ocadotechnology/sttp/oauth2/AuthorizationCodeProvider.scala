...
trait AuthorizationCodeProvider[UriType, F[_]] {
  ...
  /** Returns token details wrapped in effect
    ...
    */
  def authCodeToToken[TokenType <: OAuth2TokenResponse.Basic: EntityDecoder](authCode: String): F[TokenType]
  ...
  /** Performs the token refresh on oauth2 provider nad returns new token details wrapped in effect
    ...
    *   TokenType details containing user info and additional information
    */
  def refreshAccessToken[TokenType <: OAuth2TokenResponse.Basic: EntityDecoder](
    refreshToken: String,
...
object AuthorizationCodeProvider {
  ...
  def refinedInstance[F[_]](
    ...
    new AuthorizationCodeProvider[Refined[String, Url], F] {
      ...
      override def authCodeToToken[TT <: OAuth2TokenResponse.Basic: EntityDecoder](authCode: String): F[TT] =
        AuthorizationCode
      ...
      override def refreshAccessToken[TT <: OAuth2TokenResponse.Basic: EntityDecoder](
        refreshToken: String,
